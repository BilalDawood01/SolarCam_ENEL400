...
#endif
const int startupDelayMillis = 200; // time to wait after initialising  camera before taking photo
...
void setup() {
  ...
  s->set_hmirror(s, 1);        // Horizontal flip image 0 = disable , 1 = enable
  ...
  // Connect to Wi-Fi if required
    ...
    while (WiFi.status() != WL_CONNECTED && connAttempts < 25) {
      Serial.print(".");
      delay(500);
      connAttempts++;
    }
